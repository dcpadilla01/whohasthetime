...
1. `feeds.load_shows()` reads `podcasts.yaml` into `Show` dataclasses. `feeds.fetch_new()` parses each feed and returns `Episode`s that are not in the state DB **and** were published within `--since-hours`. Unseen episodes older than the window are silently skipped; that is what `--seed` is for when adding a new show.
